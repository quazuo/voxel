...
namespace noise
{
  ...
  namespace utils
  {
    ...
    // in out.
    inline void LinearInterpColor (const noiseutils::Color& color0, const noiseutils::Color& color1,
      float alpha, noiseutils::Color& out)
    {
...
const Color& GradientColor::GetColor (double gradientPos) const
{
  ...
  noise::utils::LinearInterpColor (color0, color1, (float)alpha, m_workingColor);
  return m_workingColor;
}
...
void WriterTER::WriteDestFile ()
{
  ...
  for (int y = 0; y < height; y++) {
    ...
    noise::uint8* pDest   = pLineBuffer;
    for (int x = 0; x < width; x++) {
      int16 scaledHeight = (int16)(floor (*pSource * 2.0f));
